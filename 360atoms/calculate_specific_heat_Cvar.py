...
def block_analysis(trace,savefile_folder,quantity='Energy'):
    ...
    SE_over_blocks = []
    block_size_all = range(1, len(trace)//int(np.sqrt(len(trace))), 10)
...
def get_atomic_info_from_PT(P,T,N,phase,case="NPT_mliap_new"):
    if "ASE" in case:
...
def get_all_atomicity_data(all_p,N,case="NPT_mliap_new"):
    molperc_full_set = []
...
def cv_lammps(case="NPT_mliap_new"):
    #prange = np.arange(140,201,10)
    #trange = np.arange(1700,2501,100)
    #LLPT_Tdict={1700:np.arange(190,201,1), 1800:np.arange(180,191,1), 1900:np.arange(160,191,2), 2000:np.arange(160,181,2), 2100:np.arange(160,181,1), 2200:np.arange(150,171,2), 2300:np.arange(146,161,2), 2400:np.arange(142,165,2), 2500:np.arange(130,165,2)}
    ...
    LLPT_Tdict = {
        ...
        1800: np.unique(np.concatenate([np.arange(183, 187, 1), np.arange(180, 191, 1)])),  # Only in LLPT_Tdict_2
    ...
    for tkel in trange:
        ...
        for pgpa in p1_to_add:
            ...
            E_reblocked = reblock(df['PotEng'].values,E_optimal_block_size)
            #E_var = (E_SE*E_optimal_block_size**0.5)**2
            E_var = np.var(df['PotEng'].values)
            #cv_blocked  = E_var/N
            #cv = np.mean(cv_blocked)
            cv = E_var/N
        ...
        plt.savefig(f'Cv_plots/Cv_liquid_{N}atoms_{tkel}K.png',dpi=300)
        plt.close()
        ...
        np.savetxt(f'Cv_plots/Cv_liquid_{N}atoms_{tkel}K_data.txt',np.array([pgpa_for_plotting,cv_for_plotting,cv_err_for_plotting]).T,header='Pressure(GPa) Cv Cv_err')
    ...
    np.savetxt(f'Cv_plots/Cv_liquid_{N}atoms_maxcv_data.txt', np.column_stack([list(trange), max_llpt_P, max_cv, max_cv_err]),
    ...
    cbar.ax.tick_params(labelsize=12)
    plt.savefig(f'Cv_plots/Cv_liquid_{N}atoms_max_pressure_vs_temp.png',dpi=300)
    plt.close()

    plt.errorbar(trange,max_cv,yerr=max_cv_err,fmt='o-')
    plt.ylabel('max(Cv) ',fontsize=14)
    plt.xlabel('Temperature (K)',fontsize=14)
    plt.savefig(f'Cv_plots/Cv_liquid_{N}atoms_temp_vs_maxcv.png',dpi=300)
    plt.close()
...
def cv_ASE():
    ...
    for tkel in trange:
        ...
        for pgpa in p_to_add:
            ...
            press_blocked = reblock(Press_val.values,2*10**4)
