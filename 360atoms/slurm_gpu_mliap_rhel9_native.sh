...
LAMMPS_BUILD_DIR=/projects/bcqo/shubhanggoswami/shubhang_builds/lammps-mace-mliap/build-gpu-aware-cuda128
LAMMPS_SRC=/projects/bcqo/shubhanggoswami/shubhang_builds/lammps-mace-mliap
LMP_EXEC=${LAMMPS_BUILD_DIR}/lmp
# HPC-X Open MPI (GPU-aware) - must be first so binary finds the MPI it was linked with
HPCX_OMPI=/opt/nvidia/hpc_sdk/Linux_x86_64/25.3/comm_libs/12.8/hpcx/hpcx-2.22.1/ompi
source /sw/rh9.4/python/miniforge3/etc/profile.d/conda.sh
conda activate lammps-mliap-rhel9
# Prefer HPC-X mpirun and MPI libs (GPU-aware build was linked with these)
export PATH="${HPCX_OMPI}/bin:${PATH}"
...
# Prepend HPC-X MPI and LAMMPS build dir so the GPU-aware binary finds its libs (cleanup above can remove module paths)
export LD_LIBRARY_PATH="${HPCX_OMPI}/lib:${LAMMPS_BUILD_DIR}:${LD_LIBRARY_PATH}"
