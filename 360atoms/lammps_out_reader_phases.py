...
def merge_and_dump_lammps(P,T,N,phase,case):
    all_data = natsorted(os.listdir(gen_data_path(P,T,N,phase,case)))
    ...
    FO = open(gen_data_path(P,T,N,phase,case)+f"analysis/P{P}T{T}M29_merged_lammps_out.csv",'w')
    header_=[]
    sim_out_array=[]
    FO_header_write_count = False
    for file_name in simout_filelist:
        with open(gen_data_path(P,T,N,phase,case)+file_name) as fin:
    ...
    np.save(gen_data_path(P,T,N,phase,case)+f"analysis/P{P}T{T}M29_merged_lammps_out",sim_out_array)

    #return sim_out_array
...
def main():
    ...
    parser.add_argument('-n','--num_atoms',dest='num_atoms',type=int,help='number of atoms',required=True)
    ...
    merge_and_dump_lammps(press,tkel,N,phase,case)
