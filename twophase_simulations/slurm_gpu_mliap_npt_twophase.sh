#!/bin/sh
# NPT runner for two-phase coexistence scans.
# Required env: temp, press (GPa), cfgfile, ROOT (twophase_simulations root)
# Optional: infile (defaults to NPT.twophase input)
module purge
...
infile=${infile:-$ROOT/lammps_inputs/in.mace.mlip.NPT.twophase.txt}
...
echo "temp=$temp press_GPa=$press pressure_bar=$pressure cfgfile=$cfgfile"
echo "infile=$infile cwd=$(pwd)"
